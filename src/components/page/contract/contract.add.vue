...
export default {
  ...
  watch: {
    'infoQueryObj.guaranteeCycle': {
      handler (value) {
        if (value) {
          this.infoQueryObj.endDate = moment(this.infoQueryObj.beginDate).add(value, 'month').format('YYYY-MM-DD HH:mm:ss')
        }
      },
      immediate: true
    },
    'infoQueryObj.beginDate': {
      handler (value) {
        if (value) {
          this.infoQueryObj.endDate = moment(value).add(this.infoQueryObj.guaranteeCycle, 'month').format('YYYY-MM-DD HH:mm:ss')
        }
      },
      immediate: true
    }
